"""
Sistema de Aprendizado Contínuo CWB Hub - Versão Corrigida
...
class ContinuousLearningSystem:
    ...
    async def analyze_session_performance(
        self,
        session: Any,  # CollaborationSession
        ...
        # Analisar performance por agente (versão simplificada)
        agent_responses = getattr(session, 'agent_responses', [])
        agent_performances = {}
        
        for response in agent_responses:
            agent_id = getattr(response, 'agent_id', 'unknown')
        ...
        for agent_id, perf in agent_performances.items():
            if perf["response_count"] > 0:
                ...
                if user_satisfaction is not None:
                    perf["response_quality"] *= user_satisfaction
    ...
    async def adapt_agent_behavior(
        ...
        """
        if not hasattr(self, 'agents') or agent_id not in self.agents:
            return False
        ...
        try:
            ...
            if "collaboration_style" in adaptation_data:
                if hasattr(agent, 'collaboration_preferences'):
                    agent.collaboration_preferences["style"] = adaptation_data["collaboration_style"]
                    adaptations_applied.append("collaboration_style")
            
            # Adaptação de comunicação
            if "communication_style" in adaptation_data:
                if hasattr(agent, 'collaboration_preferences'):
                    agent.collaboration_preferences["communication"] = adaptation_data["communication_style"]
